import { LitElement, html, until } from './app-main.js';
/**
@license
...
document.addEventListener('touchend', ignoreMouse, SUPPORTS_PASSIVE ? {
  passive: true
} : false); /** @type {!Object<string, !GestureRecognizer>} */
...
const html$1 = function html$$1(strings, ...values) {
...
class WorksPage extends LitElement {
  render() {
    return html`
            ...
            <div id="space">
                ${until(this.fetchRepos().then(repos => repos.map(repo => this.renderCard(repo))), html`<div>
                        <paper-spinner-lite active></paper-spinner-lite>
                    </div>`)}
            </div>
        `;
  ...
  fetchRepos() {
    return fetch('https://api.github.com/users/ryanafrish7/repos').then(response => response.json()).then(repos => {
      return repos.filter(repo => reposToShow.hasOwnProperty(repo.full_name)).map(repo => Object.assign(repo, reposToShow[repo.full_name])).sort((lhs, rhs) => rhs.watchers_count - lhs.watchers_count);
    });
  }
